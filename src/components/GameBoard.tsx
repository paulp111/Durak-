...
const GameBoard: React.FC = () => {
  ...
  const [turn, setTurn] = useState<"player" | "opponent1" | "opponent2">(
    "player"
  );
  ...
  const aiMakeMove = async () => {
    if (turn === "opponent1") {
      ...
      if (validCard) {
        ...
        setOpponent1Hand(
          opponent1Hand.filter((c) => c.code !== validCard.code)
        );
      }
      setTurn("opponent2");
      ...
      if (validCard) {
        handleDefend(validCard);
        setOpponent2Hand(
          opponent2Hand.filter((c) => c.code !== validCard.code)
        );
      }
      setTurn("player");
    }
  ...
  const isValidMove = (card: Card) => {
    ...
    if (
      card.suit === lastAttack.attacker.suit &&
      card.value > lastAttack.attacker.value
    ) {
      return true;
    }
    if (
      card.suit === trumpCard?.suit &&
      lastAttack.attacker.suit !== trumpCard.suit
    ) {
      return true;
    }
    return false;
  ...
  const endTurn = async () => {
    if (
      playerHand.length === 0 ||
      opponent1Hand.length === 0 ||
      opponent2Hand.length === 0
    ) {
      checkGameOver();
      return;
    }
            ...
            <div className="table-cards flex flex-wrap card-place">
              {table.map((entry, index) => (
                <div key={index} className="flex items-center mb-2">
                  <img
                    src={entry.attacker.image}
                    alt={entry.attacker.code}
                    className="w-16 h-24"
                  />
                  {entry.defender && (
                    <img
                      src={entry.defender.image}
                      alt={entry.defender.code}
                      className="w-16 h-24 ml-2"
                    />
                  )}
                </div>
              ))}
